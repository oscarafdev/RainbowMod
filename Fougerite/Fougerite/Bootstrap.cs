...
namespace Fougerite
{
    ...
    public class Bootstrap : Facepunch.MonoBehaviour
    {
        ...
        /// </summary>
        public static string SaveNotification = "Estamos guardando los datos del servidor, por favor espere un momento.";
        /// <summary>
        ...
        public static void AttachBootstrap()
        {
            try
            {
                ...
                Debug.Log(string.Format("<><[ Rainbow v{0} ]><>", Version));
            }
            catch (Exception ex)
            {
                Debug.LogException(ex);
                Debug.Log("Error al cargar Rainbow!");
            }
        }
        ...
        public bool ApplyOptions()
        {
            ...
            RainbowConfig rainbowConfig = Preferences.GetRainbowConfig();
            if (rainbowConfig.Enabled == false) 
            {
                Debug.Log("Rainbow esta desactivado, no se cargarán plugins.");
                return false;
            }
            if (rainbowConfig.RemovePlayersFromCache != null)
            {
                bool removePlayersFromCache = rainbowConfig.RemovePlayersFromCache.HasValue ? rainbowConfig.RemovePlayersFromCache.Value: false;
                CR = removePlayersFromCache;
            }
            ...
            if (!RustChat)
            {
                Logger.LogWarning("[RustChat] El chat por defecto de Rust está deshabilitado por los metodos Player.Message.");
            }
            ...
            else
            {
                ...
                Logger.LogWarning("[RustDecay] Se estableció el Decay por defecto. (No se encontraron cambios en la configuración)");
            }
            if (EnableDefaultRustDecay)
            {
                NetCull.Callbacks.beforeEveryUpdate += new NetCull.UpdateFunctor(EnvDecay.Callbacks.RunDecayThink);
                Logger.LogWarning("[RustDecay] Se estableció el Decay por defecto.");
            }
            else
            {
                Logger.LogWarning("[RustDecay] El Decay por defecto esta deshabilitado.");
            }
            return true;
        }
        ...
        private void OnIgnoredChanged(object sender, FileSystemEventArgs e)
        {
            ...
            Loom.QueueOnMainThread(() => {
                Logger.Log("[IgnoredPluginsWatcher] Se detectaron nuevos plugins ignorados, recargando lista. ");
            });
        }
