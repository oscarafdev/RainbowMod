        ...
        public void ShowMessage(string arg)
        {
            if (this.IsOnline)
            {
                ...
                if (s.Length <= 100)
                {
                    if (Bootstrap.RustChat)
                    {
                        this.SendCommand("chat.add " + Facepunch.Utility.String.QuoteSafe(arg));
                    }

                    if (Bootstrap.RPCChat)
                    {
                        string text = Facepunch.Utility.String.QuoteSafe(arg);
                        uLink.NetworkView.Get(this.PlayerClient.networkView)
                            .RPC(Bootstrap.RPCChatMethod, this.NetworkPlayer, text);
                    }
                }
                else
                {
                    ...
                    foreach (var x in Util.GetUtil().SplitInParts(arg, 100))
                    {
                        if (i == 1)
                        {
                            if (Bootstrap.RustChat)
                            {
                                this.SendCommand("chat.add " +  Facepunch.Utility.String.QuoteSafe(lastcolor + x));
                            }

                            if (Bootstrap.RPCChat)
                            {
                                string text = Facepunch.Utility.String.QuoteSafe(lastcolor + x);
                                uLink.NetworkView.Get(this.PlayerClient.networkView)
                                    .RPC(Bootstrap.RPCChatMethod, this.NetworkPlayer, text);
                            }
                        }
                        else
                        {
                            if (Bootstrap.RustChat)
                            {
                                this.SendCommand("chat.add " + Facepunch.Utility.String.QuoteSafe(x));
                            }

                            if (Bootstrap.RPCChat)
                            {
                                string text = Facepunch.Utility.String.QuoteSafe(x);
                                uLink.NetworkView.Get(this.PlayerClient.networkView)
                                    .RPC(Bootstrap.RPCChatMethod, this.NetworkPlayer, text);
                            }
                        }
