...
namespace Fougerite
{
    ...
    public class Server
    {
        private const string V = "Rainbow";
        ...
        public string server_message_name = V;
        ...
        public void BanPlayer(Fougerite.Player player, string Banner = "Consola", string reason = "Fuiste baneado del servidor.", Fougerite.Player Sender = null, bool AnnounceToServer = false)
        {
            ...
            if (player.IsOnline && !player.IsDisconnecting)
            {
                ...
                player.Message(red + " Baneado por: " + Banner);
                player.Disconnect();
            }
            if (Sender != null)
            {
                Sender.Message("Baneaste a " + player.Name);
                Sender.Message("IP de "+player.Name+": " + player.IP);
                Sender.Message("ID de " + player.Name + ": " + player.SteamID);
            }
            if (!AnnounceToServer)
            {
                foreach (Fougerite.Player pl in Players.Where(pl => pl.Admin || pl.Moderator))
                {
                    pl.Message(red + player.Name + white + " fue baneado por: " + green + Banner);
                    pl.Message(red + " Motivo: " + reason);
                }
            }
            else
            {
        ...
        public void BanPlayerIPandID(string ip, string id, string name = "1", string reason = "Fuiste baneado.", string adminname = "Anonimo")
        {
